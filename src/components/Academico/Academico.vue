<template>
  <div class="container">
    <h2 class="my-4 text-center">Académicos</h2>
    <router-link to="/CrearAcademico" class="btn btn-outline-primary">
      Agregar Académico
    </router-link>
    ...
    <div class="table-responsive">
      <table class="custom-table">
        ...
        <tbody>
          <tr v-for="academico in academicos" :key="academico.pkAcademico">
            <td class="custom-table-cell">{{ academico.planEstu }}</td>
            <td>
              <div class="custom-actions" style="margin: 3px">
                <button
                  class="btn btn-outline-success"
                  @click="abrirModal(academico)"
                  :data-id="`academico-${academico.pkAcademico}`"
                >
                  Editar
                </button>
                <button
    ...
    <EditarAcademico ref="editarAcademico" />
  </div>
</template>
  ...
  methods: {
    ...
    eliminarElementoConfirmado() {
      if (this.elementoAEliminar) {
        const id = this.elementoAEliminar.pkAcademico;

        axios
          .delete(`https://localhost:7294/Academico?id=${id}`)
          .then((response) => {
    ...
    async abrirModal(academico) {
      console.log("ID del académico a editar:", academico.pkAcademico);

      await this.$refs.editarAcademico.obtenerAcademicoParaEdicion(
        academico.pkAcademico
      );

      console.log("Mostrar modal");
      this.$refs.editarAcademico.mostrarModal = true;
    },
  },
